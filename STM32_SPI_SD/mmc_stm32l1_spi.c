...
//#define SPI_CH	2	/* SPI channel to use = 1: SPI1, 11: SPI1/remap, 2: SPI2 */
...
#define SDTIMEOUT	1000

#define CS_HIGH()	HAL_GPIO_WritePin(SD1_CS_GPIO_Port, SD1_CS_Pin, GPIO_PIN_SET)
#define CS_LOW()	HAL_GPIO_WritePin(SD1_CS_GPIO_Port, SD1_CS_Pin, GPIO_PIN_RESET)
...
//	while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY);
	HAL_SPI_TransmitReceive(&hspi3, &dat, &rxdat, sizeof (BYTE), SDTIMEOUT);
	return (BYTE) rxdat;
}
...
static uint8_t SPI_RxByte(void)
{
	...
	while(!__HAL_SPI_GET_FLAG(&hspi3, SPI_FLAG_TXE));
	//while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY);
	HAL_SPI_TransmitReceive(&hspi3, &dummy, &data, 1, SDTIMEOUT);
...
)
{
	...
	//while(!__HAL_SPI_GET_FLAG(&hspi3, SPI_FLAG_TXE));
	while ((HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY));
	//HAL_SPI_Receive(&hspi3, buff, btr, btr*SDTIMEOUT);
	HAL_SPI_TransmitReceive(&hspi3, &dummy, (uint8_t*)buff, btr, btr*SDTIMEOUT);//dummy应该是一个btr大小的数组才可以*/
	do {
		SPI_RxBytePtr(buff++);
	} while(btr--);
}
...
)
{
	while(!__HAL_SPI_GET_FLAG(&hspi3, SPI_FLAG_TXE));
	//while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY);
	HAL_SPI_Transmit(&hspi3, (uint8_t*)buff, btx, btx*SDTIMEOUT);
}
#endif
